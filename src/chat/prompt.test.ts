...
test('殿堂・禁止カードに制限バッジが付与される', () => {
  const msgs = buildMessages({
    question: '制限カードは？',
    retrieval: {
      cards: [
        { id:'dm10-036', name:'母なる大地', cardType:'呪文', cost:3, power:null, civilizations:['自然'], races:[], rarity:'R', text:'…', printings:[] },
        { id:'dm10-s05', name:'無双竜機ボルバルザーク', cardType:'クリーチャー', cost:7, power:6000, civilizations:['火','自然'], races:[], rarity:'SR', text:'…', printings:[] },
      ],
      recipes: [], meta: [], knowledge: []
    },
    history: [],
  })
  const user = msgs[msgs.length - 1].content
  assert.match(user, /《母なる大地》 \[クラシック08枠制限/)
  assert.match(user, /《無双竜機ボルバルザーク》 \[クラシック08禁止/)
})
